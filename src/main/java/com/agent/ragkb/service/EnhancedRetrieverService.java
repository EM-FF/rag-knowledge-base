...
public class EnhancedRetrieverService {
    ...
    private final com.agent.ragkb.repository.DocChunkRepository chunkRepository;
    ...
    public List<HybridRetrieverService.ScoredChunk> retrieveWithHyde(
        ...
        List<com.agent.ragkb.entity.DocChunk> hydeResults = kbIds.stream()
        ...
        Map<Long, com.agent.ragkb.entity.DocChunk> chunkMap = new HashMap<>();
        ...
        for (int rank = 0; rank < hydeResults.size(); rank++) {
            com.agent.ragkb.entity.DocChunk chunk = hydeResults.get(rank);
            double rrfScore = 1.0 / (RRF_K + rank + 1);
